...
pub fn generate_bindings(config: &Config, ci: &ComponentInterface) -> Result<Bindings> {
    let header = BridgingHeader::new(config, ci)
        .render()
        .context("failed to render Golang bridging header")?;
    let library = GolangWrapper::new(config.clone(), ci)
        .render()
        .context("failed to render Golang library")?;
    Ok(Bindings {
        library,
        header,
    })
...
#[derive(Template)]
#[template(syntax = "go", escape = "none", path = "wrapper.got")]
pub struct GolangWrapper<'a> {
    config: Config,
...
impl<'a> GolangWrapper<'a> {
    ...
    pub fn initialization_fns(&self) -> Vec<String> {
        self.ci
            .iter_types()
            .filter_map(|t| t.initialization_fn(&GolangCodeOracle))
            .collect()
    }
}
...
#[derive(Template)]
#[template(syntax = "go", escape = "none", path = "Types.got")]
...
/// code from [`GolangWrapper`].
#[derive(Template)]
#[template(syntax = "c", escape = "none", path = "HeaderTemplate.ht")]
...
#[derive(Clone)]
pub struct GolangCodeOracle;

impl GolangCodeOracle {
    // Map `Type` instances to a `Box<dyn CodeType>` for that type.
    //
    // There is a companion match in `templates/Types.got` which performs a similar function for the
    // template code.
    //
    //   - When adding additional types here, make sure to also add a match arm to the `Types.got` template.
    ...
    //// Get the idiomatic Golang rendering of a converter name.
    // fn ffi_converter_name(&self, codetype: &impl CodeType) -> String {
    //     codetype.canonical_name(self).to_upper_camel_case()
    // }
}

impl CodeOracle for GolangCodeOracle {
    fn find(&self, type_: &TypeIdentifier) -> Box<dyn CodeType> {
        self.create_code_type(type_.clone())
    }

    /// Get the idiomatic Golang rendering of a class name (for enums, records, errors, etc).
    fn class_name(&self, nm: &str) -> String {
        nm.to_string().to_upper_camel_case()
    }

    /// Get the idiomatic Golang rendering of a function name.
    fn fn_name(&self, nm: &str) -> String {
        format!("{}", nm.to_string().to_upper_camel_case())
    }

    /// Get the idiomatic Golang rendering of a variable name.
    fn var_name(&self, nm: &str) -> String {
        format!("{}", nm.to_string().to_lower_camel_case())
    }

    /// Get the idiomatic Golang rendering of an individual enum variant.
    fn enum_variant_name(&self, nm: &str) -> String {
        format!("{}", nm.to_string().to_lower_camel_case())
    }

    /// Get the idiomatic Golang rendering of an exception name.
...
pub mod filters {
    ...
    fn oracle() -> &'static GolangCodeOracle {
        &GolangCodeOracle
    }
    ...
    /// Get the Golang syntax for representing a given low-level `FfiType`.
    ...
    /// Get the idiomatic Golang rendering of an individual enum variant.
    pub fn enum_variant_golang(nm: &str) -> Result<String, askama::Error> {
